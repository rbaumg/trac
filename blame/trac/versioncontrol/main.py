...
class Repository(object):
    ...
    def get_node(self, path, rev=None):
        ...
        If the `rev` parameter is specified, the version of the node at that
        revision is returned, otherwise the latest version of the node is
        returned.
        """
        raise NotImplementedError
    ...
    def rev_older_than(self, rev1, rev2):
        """Return `True` if `rev1` is older than `rev2`."""
        raise NotImplementedError

    def get_youngest_rev_in_cache(self, db):
        """Return the youngest revision currently cached.

        The way revisions are sequenced is version control specific. The default
        implementation assumes that the revisions are sequenced in time.
        """
        cursor = db.cursor()
    ...
    def get_path_history(self, path, rev=None, limit=None):
        """Retrieve all the revisions containing this path (no newer than
        `rev`).
    ...
    def normalize_path(self, path):
        """Return a canonical representation of path in the repository."""
...
class Node(object):
    ...
    def get_content(self):
        ...
        This method will return `None` for directories. The returned object
        should provide a `read([len])` function.
        """
        raise NotImplementedError

    def get_entries(self):
        """Generator that yields the immediate child entries of a directory, in
        no particular order.
    ...
    def get_history(self, limit=None):
        ...
        revision in which the node was changed.
        
        This generator will follow copies and moves of a node (if the underlying
        version control system supports that), which will be indicated by the
        first element of the tuple (i.e. the path) changing.
        """
        raise NotImplementedError

    def get_properties(self):
        """Return a dictionary containing the properties (meta-data) of the
        node.
...
class Changeset(object):
    """A set of changes in a repository."""
    ...
    def get_changes(self):
        ...
        or `Changeset.MOVE`, and `kind` is one of `Node.FILE` or
        `Node.DIRECTORY`.
        """
        raise NotImplementedError
...
class PermissionDenied(Exception):
    """Exception raised by an authorizer if the user has insufficient
    permissions to view a specific part of the repository.
    """
