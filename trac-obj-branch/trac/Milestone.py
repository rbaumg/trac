...
class Milestone(Module):
    ...
    def create_milestone(self, req, name, due=0, completed=0, description=''):
        ...
        object_factory(self.env, 'milestone', name).replace_xrefs_from_wiki(self.env, self.db, 'description', description)
        self.db.commit()
    ...
    def delete_milestone(self, req, id):
        ...
        if req.args.has_key('delete'):
            ...
            object_factory(self.env, 'milestone', id).delete_xrefs(self.db, 'description')
            self.db.commit()
            req.redirect(self.env.href.roadmap())
        else:
            req.redirect(self.env.href.milestone(id))
    ...
    def render(self, req):
        ...
        object_factory(self.env, 'milestone', id).add_cross_refs(self.db, req)
